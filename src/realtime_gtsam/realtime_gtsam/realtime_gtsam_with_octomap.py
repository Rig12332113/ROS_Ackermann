...
class RGBD_Node(Node):
    def __init__(self):
        ...
        self.kf_cloud_pub = self.create_publisher(
            PointCloud2,
            "/rgbd_slam/keyframe_cloud",
            10,
        )
        ...
        self.min_loop_inliers = 40
        self.max_loop_trans_error = 6.0
        self.max_loop_rot_error = 1.5
        ...
        self.local_tracking_min_inliers = 50
        self.local_tracking_max_translation = 2.50
        self.local_tracking_max_rotation = 2.00

        self.map_sample_step = 6
        self.map_depth_min = 0.2
        self.map_depth_max = 8.0
        self.max_points_per_keyframe = 8000
    ...
    def rgbdCallBack(self, rgb_msg, depth_msg):
        ...
        if succ:
            ...
            self.publish_single_keyframe_cloud(new_kf)
    ...
    def publish_single_keyframe_cloud(self, kf):
        ...
        for v in range(0, h, self.map_sample_step):
            for u in range(0, w, self.map_sample_step):
                ...
                p_camera = np.array([x, y, z, 1.0], dtype=np.float64)
        ...
        self.kf_cloud_pub.publish(cloud_msg)

        self.get_logger().info(
            f"Published keyframe cloud: KF {kf.id}, points={len(points)}"
        )
    ...
    def try_add_loop_closure(self, keyframes, new_kf):
        self.get_logger().info(
            f"Try loop closure for KF {new_kf.id}, total_keyframes={len(keyframes)}"
        )
        ...
        for kf in searchable_keyframes:
            ...
            score = inliers - 30.0 * trans_error - 20.0 * rot_error
